...
public class CsrfTokenServiceTest {
  ...
  private CsrfConfigurationProvider mockCsrfProtectionDefinitionProvider;
  private org.pentaho.platform.web.csrf.filter.CsrfTokenService csrfTokeResource;
  ...
  public void setUp() {
    ...
    this.mockCsrfProtectionDefinitionProvider = mock( CsrfConfigurationProvider.class );

    this.csrfTokeResource = spy( new org.pentaho.platform.web.csrf.filter.CsrfTokenService( this.mockCsrfProtectionDefinitionProvider ) );
  ...
  public void testWhenNoCsrfTokenInRequest() throws Exception {
    ...
    csrfTokeResource.init( cfg );

    csrfTokeResource.doFilter( this.mockRequest, this.mockResponse, this.filterChain );
  ...
  public void testWhenCsrfTokenThenCsrfResponseHeaders() throws Exception {
    ...
    verify( mockResponse, once() ).setHeader( eq( CsrfTokenService.RESPONSE_HEADER_HEADER ), eq( RESPONSE_HEADER_VALUE ) );
    verify( mockResponse, once() ).setHeader( eq( CsrfTokenService.RESPONSE_HEADER_PARAM ), eq( RESPONSE_PARAM_VALUE ) );
    verify( mockResponse, once() ).setHeader( eq( CsrfTokenService.RESPONSE_HEADER_TOKEN ), eq( RESPONSE_TOKEN_VALUE ) );
  }
