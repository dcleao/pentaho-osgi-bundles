 ...
 */
public class CsrfClient implements ICsrfClient {
  ...
  /** @inheritDoc */
  public ICsrfToken getToken( URI tokenServiceUri, CookieHandler cookieHandler, URI protectedServiceUri ) {
    ...
    Invocation.Builder builder = client.target( tokenServiceUri )
      .queryParam( ICsrfService.QUERY_PARAM_URL, protectedServiceUri )
      .request();
    ...
    String token = response.getHeaderString( ICsrfService.RESPONSE_HEADER_TOKEN );
    if ( token == null || token.length() == 0 ) {
      return null;
    }

    String header = response.getHeaderString( ICsrfService.RESPONSE_HEADER_HEADER );
    String parameter = response.getHeaderString( ICsrfService.RESPONSE_HEADER_PARAM );
