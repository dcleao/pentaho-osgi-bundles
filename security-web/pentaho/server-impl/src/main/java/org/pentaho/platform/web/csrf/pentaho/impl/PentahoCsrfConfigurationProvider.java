...
import org.hitachivantara.security.web.api.csrfold.CsrfConfiguration;
...
public class PentahoCsrfConfigurationProvider implements PentahoCsrfProtection {
  ...
  @Override
  public Collection<CsrfConfiguration> getProtectionDefinitions() {
    // Should be empty, when globally disabled, but just in case...
    return isEnabled()
      ? PentahoSystem.getAll( CsrfConfiguration.class )
      : Collections.emptyList();
  }

  @Override
  public void addCsrfConfigurationListener( final @Nonnull Runnable listener ) {
    getPluginManager().addPluginManagerListener( listener::run );
  }
